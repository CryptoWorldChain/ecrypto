...
package org.brewchain.ecrypto.address.btc;
 ...
 */
public class BTCKey implements EncryptableItem {
    private static final Logger log = LoggerFactory.getLogger(BTCKey.class);

    /** Sorts oldest keys first, newest last. */
    public static final Comparator<BTCKey> AGE_COMPARATOR = new Comparator<BTCKey>() {

        @Override
        public int compare(BTCKey k1, BTCKey k2) {
    ...
    public static final Comparator<BTCKey> PUBKEY_COMPARATOR = new Comparator<BTCKey>() {
     ...
     */
    public BTCKey(SecureRandom secureRandom) {
    ...
    protected BTCKey(@Nullable BigInteger priv, ECPoint pub) {
        this(priv, new LazyECPoint(checkNotNull(pub)));
    }

    protected BTCKey(@Nullable BigInteger priv, LazyECPoint pub) {
        if (priv != null) {
     ...
     */
    public static BTCKey fromASN1(byte[] asn1privkey) {
     ...
     */
    public static BTCKey fromPrivate(BigInteger privKey) {
     ...
     * compressed or not.
     */
    public static BTCKey fromPrivate(BigInteger privKey, boolean compressed) {
        ECPoint point = publicPointFromPrivate(privKey);
        return new BTCKey(privKey, getPointWithCompression(point, compressed));
    }
     ...
     * public key is compressed.
     */
    public static BTCKey fromPrivate(byte[] privKeyBytes) {
     ...
     * compressed or not.
     */
    public static BTCKey fromPrivate(byte[] privKeyBytes, boolean compressed) {
     ...
     */
    public static BTCKey fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new BTCKey(priv, pub);
    }
     ...
     */
    public static BTCKey fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        checkNotNull(priv);
        checkNotNull(pub);
        return new BTCKey(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     */
    public static BTCKey fromPublicOnly(ECPoint pub) {
        return new BTCKey(null, pub);
    }
     ...
     */
    public static BTCKey fromPublicOnly(byte[] pub) {
        return new BTCKey(null, CURVE.getCurve().decodePoint(pub));
    }
     ...
     */
    public BTCKey decompress() {
        if (!pub.isCompressed())
            return this;
        else
            return new BTCKey(priv, decompressPoint(pub.get()));
    }
     ...
     */
    @Deprecated
    public BTCKey(@Nullable byte[] privKeyBytes, @Nullable byte[] pubKey) {
     ...
     */
    @Deprecated
    public BTCKey(EncryptedData encryptedPrivateKey, byte[] pubKey, KeyCrypter keyCrypter) {
     ...
     */
    public static BTCKey fromEncrypted(EncryptedData encryptedPrivateKey, KeyCrypter crypter, byte[] pubKey) {
        BTCKey key = fromPublicOnly(pubKey);
     ...
     */
    @Deprecated
    public BTCKey(@Nullable BigInteger privKey, @Nullable byte[] pubKey, boolean compressed) {
     ...
     */
    @Deprecated
    private BTCKey(@Nullable BigInteger privKey, @Nullable byte[] pubKey) {
     ...
     * @throws org.BTCKey.core.ECKey.MissingPrivateKeyException if the private key is missing or encrypted.
     */
    public byte[] toASN1() {
        try {
    ...
    public static class ECDSASignature {
        ...
        /**
         * Returns true if the S component is "low", that means it is below {@link BTCKey#HALF_CURVE_ORDER}. See <a
...
//                Properties.removeThreadOverride("org.spongycastle.asn1.allow_unsafe_integer");
     ...
     * usually encoded using ASN.1 format, so you want {@link BTCKey.ECDSASignature#toASN1()}
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Bitcoin protocol, they are
     * usually encoded using DER format, so you want {@link BTCKey.ECDSASignature#encodeToDER()}
     * instead. However sometimes the independent components can be useful, for instance, if you're doing to do further
     * EC maths on them.
     *
     * @param aesKey The AES key to use for decryption of the private key. If null then no decryption is required.
     * @throws KeyCrypterException if there's something wrong with aesKey.
     * @throws BTCKey.MissingPrivateKeyException if this key cannot sign because it's pubkey only.
     */
    ...
    public boolean verify(byte[] hash, byte[] signature) {
        return BTCKey.verify(hash, signature, getPubKey());
    }
    ...
    public boolean verify(Sha256Hash sigHash, ECDSASignature signature) {
        return BTCKey.verify(sigHash.getBytes(), signature, getPubKey());
    }
    ...
    public void verifyOrThrow(Sha256Hash sigHash, ECDSASignature signature) throws SignatureException {
        if (!BTCKey.verify(sigHash.getBytes(), signature, getPubKey()))
            throw new SignatureException();
    }
    ...
    private static BTCKey extractKeyFromASN1(byte[] asn1privkey) {
        ...
        try {
            ...
            BTCKey key = new BTCKey(privkey, null, compressed);
     ...
     */
    public static BTCKey signedMessageToKey(String message, String signatureBase64) throws SignatureException {
        ...
        int recId = header - 27;
        BTCKey key = BTCKey.recoverFromSignature(recId, sig, messageHash, compressed);
        if (key == null)
    ...
    /**
     * Convenience wrapper around {@link BTCKey#signedMessageToKey(String, String)}. If the key derived from the
    ...
    public void verifyMessage(String message, String signatureBase64) throws SignatureException {
        BTCKey key = BTCKey.signedMessageToKey(message, signatureBase64);
    ...
    public byte findRecoveryId(Sha256Hash hash, ECDSASignature sig) {
        byte recId = -1;
        for (byte i = 0; i < 4; i++) {
            BTCKey k = BTCKey.recoverFromSignature(i, sig, hash, isCompressed());
     ...
     */
    @Nullable
    public static BTCKey recoverFromSignature(int recId, ECDSASignature sig, Sha256Hash message, boolean compressed) {
        ...
        return BTCKey.fromPublicOnly(q.getEncoded(compressed));
    }
     ...
     * @throws org.BTCKey.core.ECKey.MissingPrivateKeyException if the private key bytes are missing/encrypted.
     */
...
//    }
     ...
     * @return encryptedKey
     */
    public BTCKey encrypt(KeyCrypter keyCrypter, KeyParameter aesKey) throws KeyCrypterException {
        checkNotNull(keyCrypter);
        final byte[] privKeyBytes = getPrivKeyBytes();
        EncryptedData encryptedPrivateKey = keyCrypter.encrypt(privKeyBytes, aesKey);
        BTCKey result = BTCKey.fromEncrypted(encryptedPrivateKey, keyCrypter, getPubKey());
     ...
     */
    public BTCKey decrypt(KeyCrypter keyCrypter, KeyParameter aesKey) throws KeyCrypterException {
        checkNotNull(keyCrypter);
        ...
        BTCKey key = BTCKey.fromPrivate(unencryptedPrivateKey);
        if (!isCompressed())
            key = key.decompress();
     ...
     * @param aesKey The KeyParameter with the AES encryption key (usually constructed with keyCrypter#deriveKey and cached).
     */
    public BTCKey decrypt(KeyParameter aesKey) throws KeyCrypterException {
     ...
     */
    public BTCKey maybeDecrypt(@Nullable KeyParameter aesKey) throws KeyCrypterException {
     ...
     */
    public static boolean encryptionIsReversible(BTCKey originalKey, BTCKey encryptedKey, KeyCrypter keyCrypter, KeyParameter aesKey) {
        try {
            BTCKey rebornUnencryptedKey = encryptedKey.decrypt(keyCrypter, aesKey);
...
//    }
    ...
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || !(o instanceof BTCKey)) return false;
        BTCKey other = (BTCKey) o;
